...
async def scheduled_pregame_predictor():
    ...
    Uses comprehensive line movement analysis across ALL markets - NO LLM REQUIRED!
    
    Features:
    - Studies COMPLETE line movement from opening to 1 hour before game
    - Analyzes ML, Spread, and Totals markets separately
    - Detects sharp money vs public money patterns
    - Identifies steam moves and reverse line movement
    - Cross-market signal analysis
    - Squad analysis with injury impact
    - H2H historical analysis
    - Venue and home advantage factors
    - Only recommends when 3+ factors align
    - Picks can be ML, Spread, OR Total based on strongest signal
    """
        ...
        try:
            ...
            for sport_key in sports_to_analyze:
                try:
                    ...
                    for event in events:
                        try:
                            ...
                            if window_start <= commence_time <= window_end:
                                ...
                                # This game is ~1 hour away - time for V5 analysis!
                                logger.info(f"📊 V5 PRE-GAME ANALYSIS: {home_team} vs {away_team} "
                                          f"(starts in {(commence_time - now).total_seconds() / 60:.0f} min)")
                                
                                # 1. Get comprehensive matchup data (form, H2H, venue)
                                matchup_data = await get_comprehensive_matchup_data(event, sport_key)
                                
                                # 2. Get squad data (injuries, lineups)
                                squad_data = await get_matchup_context(home_team, away_team, sport_key)
                                
                                # 3. Get COMPLETE line movement history (ALL markets)
                                line_history = await db.odds_history.find(
                                ...
                                # 6. Run BetPredictor V5 comprehensive analysis
                                v5_prediction = await generate_v5_prediction(
                                    event=event,
                                    sport_key=sport_key,
                                ...
                                if v5_prediction and v5_prediction.get("has_pick"):
                                    confidence = v5_prediction.get("confidence", 0) / 100  # V5 returns 0-100
                                    
                                    if confidence >= 0.70:
                                        # Create prediction from V5 result
                                        prediction = PredictionCreate(
                                            ...
                                            away_team=away_team,
                                            commence_time=commence_str,
                                            prediction_type=v5_prediction.get("pick_type", "moneyline"),
                                            predicted_outcome=v5_prediction.get("pick_display", v5_prediction.get("pick", "")),
                                            confidence=confidence,
                                            analysis=v5_prediction.get("reasoning", ""),
                                            ai_model="betpredictor_v5",
                                            odds_at_prediction=v5_prediction.get("odds", 1.91)
                                        )
                                        
                                        await create_recommendation(prediction)
                                        predictions_made += 1
                                        
                                        logger.info(f"✅ V5 PREDICTION: {home_team} vs {away_team} - "
                                                  f"{v5_prediction.get('pick_type')}: {v5_prediction.get('pick_display', v5_prediction.get('pick'))} "
                                                  f"@ {confidence*100:.0f}% conf, factors: {v5_prediction.get('factor_count', 0)}")
                                    else:
                                        logger.info(f"⏭️ LOW CONFIDENCE: {home_team} vs {away_team} - "
                                                  f"{confidence*100:.0f}% < 70% threshold")
                                else:
                                    reason = v5_prediction.get("reasoning", "No value found") if v5_prediction else "Analysis failed"
                    ...
                    logger.error(f"Error in V5 pregame predictor for {sport_key}: {e}")
            
            if predictions_made > 0:
                logger.info(f"📊 V5 pre-game predictor complete - {predictions_made} predictions created")
            
            # Run every 10 minutes to catch all games in the 1 hour window
            await asyncio.sleep(600)
            
        except Exception as e:
            logger.error(f"Scheduled V5 pregame predictor error: {e}")
            await asyncio.sleep(120)


# BetPredictor V6 - Pre-game predictor with Advanced ML Ensemble
async def scheduled_pregame_predictor_v6():
    """
    ...
    logger.info("🚀 Started BETPREDICTOR V6 - Advanced ML ensemble analysis 1 hour before game")
                    ...
                    for event in events:
                        try:
                            ...
                            if window_start <= commence_time <= window_end:
                                ...
                                # Check if we already have a V6 prediction for this event
                                existing = await db.predictions.find_one({
                                    "event_id": event_id, 
                                    "result": "pending",
                                    "ai_model": "betpredictor_v6"
                                ...
                                if existing:
                                    logger.debug(f"Skipping {event_id} - already has V6 prediction")
                                    continue
                                
                                # This game is ~1 hour away - time for V6 analysis!
                                logger.info(f"🚀 V6 PRE-GAME ANALYSIS: {home_team} vs {away_team} "
                                          f"(starts in {(commence_time - now).total_seconds() / 60:.0f} min)")
                                ...
                                # 6. Run BetPredictor V6 advanced analysis
                                v6_prediction = await generate_v6_prediction(
                                    event=event,
                                    sport_key=sport_key,
                                    squad_data=squad_data,
                                    matchup_data=matchup_data,
                                ...
                                if v6_prediction and v6_prediction.get("has_pick"):
                                    confidence = v6_prediction.get("confidence", 0) / 100
                                    edge = v6_prediction.get("edge", 0) / 100
                                    
                                    if confidence >= 0.65:  # V6 uses 65% threshold
                                        # Create prediction from V6 result
                                        prediction = PredictionCreate(
                                            event_id=event_id,
                                            sport_key=sport_key,
                                            home_team=home_team,
                                            away_team=away_team,
                                            commence_time=commence_str,
                                            prediction_type=v6_prediction.get("pick_type", "moneyline"),
                                            predicted_outcome=v6_prediction.get("pick_display", v6_prediction.get("pick", "")),
                                            confidence=confidence,
                                            analysis=v6_prediction.get("reasoning", ""),
                                            ai_model="betpredictor_v6",
                                            odds_at_prediction=v6_prediction.get("odds", 1.91)
                                        )
                                        
                                        await create_recommendation(prediction)
                                        predictions_made += 1
                                        
                                        logger.info(f"✅ V6 PREDICTION: {home_team} vs {away_team} - "
                                                  f"{v6_prediction.get('pick_type')}: {v6_prediction.get('pick_display', v6_prediction.get('pick'))} "
                                                  f"@ {confidence*100:.0f}% conf, edge: {edge*100:.1f}%, "
                                                  f"agreement: {v6_prediction.get('model_agreement', 0):.0f}%")
                                    else:
                                        logger.info(f"⏭️ LOW CONFIDENCE: {home_team} vs {away_team} - "
                                                  f"{confidence*100:.0f}% < 65% threshold")
                                else:
                                    reason = v6_prediction.get("reasoning", "No consensus") if v6_prediction else "Analysis failed"
                                    logger.info(f"⏭️ NO PICK: {home_team} vs {away_team} - {reason[:150]}")
                            ...
                            logger.error(f"Error in V6 analysis for event: {e}")
                            import traceback
                            traceback.print_exc()
                            continue
                    
                except Exception as e:
                    logger.error(f"Error in V6 pregame predictor for {sport_key}: {e}")
            
            if predictions_made > 0:
                logger.info(f"🚀 V6 pre-game predictor complete - {predictions_made} predictions created")
            
            # Run every 10 minutes to catch all games in the 1 hour window
            await asyncio.sleep(600)
            
        except Exception as e:
            logger.error(f"Scheduled V6 pregame predictor error: {e}")
            await asyncio.sleep(120)
